...
const TOPICS = [
  { slug: 'cv', title: 'السيرة الذاتية', match: seed => seed.intent === 'cv-guide' || /سيرة ذاتية|السيره الذاتيه|cv|سي في/i.test(seed.keyword) },
  ...
  { slug: 'safety', title: 'الأمان وتجنب الاحتيال', match: seed => String(seed.intent).includes('safety') || /احتيال|نصب|وهمي|مشبوه/i.test(seed.keyword) },
...
function keywordCovered(keyword, manifest) {
  ...
  return manifest.some(item => normalizeArabic(item.keyword) === normalized || jaccard(keyword, `${item.keyword || ''} ${item.title || ''}`) >= 0.86);
}
...
function chooseKeyword(pool, manifest) {
  ...
  return [...pool].filter(item => isGuidanceScope(item.keyword)).filter(item => !keywordCovered(item.keyword, manifest)).sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0))[0] || null;
}
...
function buildPrompt(seed, manifest) {
  ...
  const previousTitles = manifest.slice(0, 30).map(item => item.title).join(' | ');
  return `أنت محرر محتوى عربي لمركز NEXT JOB الإرشادي. اكتب دليلًا عربيًا أصليًا ومفيدًا مخصصًا لليمنيين الموجودين داخل المملكة العربية السعودية، وموضوعه الأساسي: ${topic.title}.
...
4) NEXT JOB مركز إرشادي مستقل وليس مكتب توظيف أو جهة استقدام أو وسيطًا، ولا تعد القارئ بالحصول على وظيفة أو نقل الخدمات.
...
6) اجعل النص عمليًا بخطوات، أسئلة يراجعها القارئ، أخطاء شائعة، وعلامات تحذيرية عندما يناسب الموضوع.
7) استخدم لغة عربية واضحة ومهنية ومباشرة، ولا تستخدم حشو SEO.
8) لا توسع الجمهور إلى جنسيات أخرى؛ إذا ذكرت المقيمين فقل المقيمين اليمنيين.
9) لا تكرر عنوانًا قريبًا من هذه العناوين السابقة: ${previousTitles || 'لا يوجد'}.
10) المحتوى من 850 إلى 1400 كلمة تقريبًا، 5 إلى 8 أقسام، و3 إلى 5 أسئلة شائعة.
11) الخاتمة توجه القارئ إلى موضوعات NEXT JOB الإرشادية ذات الصلة، ويمكنها الإشارة إلى دليل الفرص كمصدر مساعد فقط دون ادعاء توفر شواغر معينة.
...
function validateArticle(article, seed, manifest) {
  ...
  for (const phrase of ['منصة توظيف تقنية مرخصة','مطابقة للأنظمة','وظيفة مضمونة','التوظيف مضمون','راتب مضمون','نضمن لك وظيفة','نضمن التوظيف','نضمن نقل الخدمات']) {
  ...
  for (const previous of manifest) {
    if (jaccard(title, `${previous.title || ''} ${previous.keyword || ''}`) >= 0.86) { errors.push('too similar to a published article'); break; }
  }
