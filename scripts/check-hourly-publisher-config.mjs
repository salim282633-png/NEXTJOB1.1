...
assert.ok(workflow.includes("cron: '17 */6 * * *'"), 'primary publisher must run every six hours at minute 17');
assert.ok(render.includes("schedule: '47 * * * *'"), 'Render recovery must check hourly at minute 47');
...
assert.ok(dispatcher.includes('const publishSlotMs = 6 * 60 * 60 * 1000;'), 'recovery dispatch must report the aligned six-hour slot');
