...
const INTENT_LINKS_FILE = path.join(ROOT, 'seo/intent-internal-links.json');
...
function intentLinksMarkup(meta, manifest) {
  const map = readJson(INTENT_LINKS_FILE, { pages: {} });
  const links = Array.isArray(map?.pages?.[meta.slug]) ? map.pages[meta.slug] : [];
  if (!links.length) return '';
  const knownSlugs = new Set(manifest.map(item => item.slug).filter(Boolean));
  const items = links.filter(item => knownSlugs.has(item.targetSlug));
  if (!items.length) return '';
  return `<aside class="intent-links" aria-label="خطوتك التالية في المدونة">
          <strong>خطوتك التالية</strong>
          <div>${items.map(item => `<a href="/guide/${escapeHtml(item.targetSlug)}/">${escapeHtml(item.anchor)}</a><span>${escapeHtml(item.note)}</span>`).join('')}</div>
        </aside>`;
}
...
function renderArticlePage(meta, article, manifest) {
  ...
  const intentLinks = intentLinksMarkup(meta, manifest);
...
<head>
  ...
  <style>
    .intent-links{margin:34px 0 4px;padding:20px;border:1px solid #d5e5dc;border-radius:18px;background:#f7fbf8}.intent-links>strong{display:block;margin-bottom:11px;color:var(--green-dark);font-size:15px}.intent-links>div{display:grid;gap:11px}.intent-links a{display:block;color:var(--green-dark);font-weight:900;text-decoration:underline;text-decoration-thickness:1px;text-underline-offset:4px}.intent-links span{display:block;margin-top:3px;color:var(--body);font-size:13px;line-height:1.8}.intent-links a+span:not(:last-child){padding-bottom:10px;border-bottom:1px solid #e3ece7}
  </style>
</head>
<body>
