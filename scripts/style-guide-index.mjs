  ...
  return `
    <article class="article-card" data-search="${escapeHtml(searchable)}">
      ...
      <div class="card-footer">
        <span>${item.wordCount ? `${escapeHtml(item.wordCount)} كلمة تقريبًا` : 'مقال إرشادي'}</span>
        <a class="read" href="/guide/${escapeHtml(item.slug)}/">قراءة المقال <span aria-hidden="true">←</span></a>
      </div>
    </article>`;
}
...
function render(manifest) {
  ...
  const blogSchema = {
    ...
    '@type': 'Blog',
    '@id': `${SITE_URL}/guide/#blog`,
    url: `${SITE_URL}/guide/`,
    name: 'مدونة NEXT JOB',
    description: 'مدونة إرشادية لليمنيين في السعودية حول البحث عن عمل والسيرة الذاتية والمقابلات والعقود ونقل الخدمات والأمان والمسار المهني.',
    inLanguage: 'ar-SA',
...
<head>
  ...
  <script type="application/ld+json">${safeJsonForScript(blogSchema)}</script>
  <style>
  ...
  <script>
    (() => {
      ...
      const apply = () => {
        ...
        counter.textContent = visible + ' مقال';
