    ...
    <section className="py-8 sm:py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
      <div className="relative overflow-hidden rounded-[28px] border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-6 sm:p-8 shadow-[0_18px_50px_rgba(15,23,42,0.055)] flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="relative space-y-3 max-w-2xl">
          <div className="inline-flex items-center gap-2 bg-white text-emerald-800 px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-bold border border-emerald-100 shadow-sm">
            ...
            <span>باحثون عن عمل داخل السعودية</span>
          </div>
          <h2 className="text-2xl sm:text-[32px] font-black text-slate-950 font-display leading-[1.35]">
            دليل الكفاءات والباحثين عن عمل
          </h2>
          <p className="text-sm text-slate-600 leading-7 sm:leading-8 max-w-xl">
            تصفح الملفات المهنية حسب المدينة والمهارات، واطلع على الخبرات قبل التواصل المباشر مع الباحث.
          </p>
        </div>
        ...
        <button
          ...
          className="relative shrink-0 w-full md:w-auto px-5 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-2xl text-sm shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
        >
          <UserPlus className="w-4 h-4" />
          <span>أضف ملفك المهني</span>
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 sm:gap-3">
          <div className="sm:col-span-6 relative">
            ...
            <input
              ...
              placeholder="ابحث بالاسم أو المهنة أو المهارة"
              className="ui-control w-full pl-4 pr-10 py-3 bg-slate-50/70 text-sm font-semibold placeholder:font-normal"
            />
          </div>
          ...
          <div className="sm:col-span-3 relative">
            ...
            <select
              ...
              className="ui-control w-full pl-4 pr-10 py-3 bg-slate-50/70 text-sm font-semibold"
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-xl font-bold transition-all flex items-center gap-1.5 border ${filter.availableOnly ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm' : 'bg-white text-slate-700 border-slate-200 hover:border-emerald-300 hover:text-emerald-700'}`}
            >
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>متاح للمباشرة</span>
            </button>

            <button
              id="btn-filter-cand-license"
              onClick={() => setFilter(prev => ({ ...prev, hasLicenseOnly: !prev.hasLicenseOnly }))}
              className={`px-3 py-2 rounded-xl font-bold transition-all flex items-center gap-1.5 border ${filter.hasLicenseOnly ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm' : 'bg-white text-slate-700 border-slate-200 hover:border-emerald-300 hover:text-emerald-700'}`}
            >
      ...
      {!isLoading && (
        <div className="flex items-center justify-between gap-3 px-1">
          <div>
            <h3 className="text-lg sm:text-xl font-black text-slate-950 font-display">الملفات المهنية</h3>
            <p className="text-xs text-slate-500 mt-1">{filteredCandidates.length} ملف مطابق للفلاتر الحالية</p>
          </div>
        </div>
      )}
      ...
      {!isLoading && filteredCandidates.length === 0 && (
        <div className="ui-card p-8 sm:p-12 text-center max-w-xl mx-auto space-y-4">
          <div className="w-16 h-16 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center mx-auto">
            <Users className="w-8 h-8" />
          </div>
          <h3 className="text-xl font-black text-slate-950 font-display">لم يتم العثور على باحثين وفق الفلتر المحدد</h3>
          <p className="text-sm text-slate-500 leading-7">جرّب تغيير المدينة أو الكلمات المستخدمة، أو أضف ملفك المهني ليظهر لأصحاب العمل.</p>
          <button
            id="btn-post-candidate-empty"
            onClick={onOpenPostCandidate}
            className="px-5 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors inline-flex items-center gap-1.5"
          >
            <UserPlus className="w-4 h-4" />
            <span>إضافة ملفي المهني</span>
          </button>
        </div>
      )}
    </section>
