...
import { AlertTriangle, Banknote, Bookmark, Briefcase, Building2, Check, Flame, MapPin, MessageCircle, PhoneCall, Share2, ShieldAlert, ShieldCheck, Sparkles, X } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-200" role="dialog" aria-modal="true" aria-labelledby="job-detail-title">
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-md px-5 py-4 border-b flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <button onClick={() => onToggleSave(job)} className={`p-2 border rounded-xl ${isSaved ? 'text-emerald-700 bg-emerald-50' : 'text-slate-500'}`} aria-label={isSaved ? 'إزالة من المحفوظات' : 'حفظ الوظيفة'}><Bookmark className={`w-4 h-4 ${isSaved ? 'fill-emerald-600' : ''}`} /></button>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <div className="bg-slate-50 p-3 rounded-xl"><span className="text-[11px] text-slate-500 block">نقل الخدمات</span><strong className="text-sm">{job.sponsorshipTransfer ? 'ذكر المعلن أنه يتم نظاميًا عبر قوى' : 'غير محدد'}</strong></div>
          </div>

          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-950 leading-relaxed">
            <strong>تنبيه:</strong> معلومات الراتب والمزايا ونقل الخدمات مقدمة من صاحب الإعلان. NEXT JOB لا تضمن إتمام التوظيف أو نقل الخدمات، وأي إجراء يجب أن يتم عبر الجهات والمنصات الرسمية بحسب الحالة.
          </div>
          ...
          {job.requirements?.length ? <div><h3 className="font-bold mb-2">المتطلبات</h3><ul className="space-y-2">{job.requirements.map((item, index) => <li key={index} className="text-sm flex gap-2"><Check className="w-4 h-4 text-emerald-600 shrink-0" />{item}</li>)}</ul></div> : null}
          ...
          <div className="bg-slate-900 text-white p-5 rounded-3xl space-y-3">
            <h4 className="font-bold">التواصل المباشر مع صاحب الإعلان</h4>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-xs text-slate-400"><span className="flex gap-1"><AlertTriangle className="w-3.5 h-3.5 text-amber-400" />لا تدفع أي رسوم مقابل وعد بالتوظيف.</span>{onReportFraud && <button onClick={() => onReportFraud(job)} className="text-rose-400 font-bold flex gap-1"><ShieldAlert className="w-3.5 h-3.5" />إبلاغ</button>}</div>
          </div>
        </div>
      </div>
    </div>
