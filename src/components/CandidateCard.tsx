...
export const CandidateCard: React.FC<CandidateCardProps> = ({ candidate, onQuickWhatsApp, onViewCV, onReportCandidate }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-xs hover:shadow-lg transition-all">
      ...
      <div className="grid grid-cols-2 gap-2 bg-slate-50 p-2.5 rounded-xl text-xs mb-3"><span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-emerald-600" />{candidate.city}</span><span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-emerald-600" />{candidate.experienceYears}</span></div>
