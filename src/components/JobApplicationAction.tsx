...
export const JobApplicationAction: React.FC<Props> = ({ job, user, candidate }) => {
  ...
  const canUsePlatformApplication = Boolean(job.userId && user && job.userId !== user.uid);
  const applicationId = canUsePlatformApplication && user ? `${job.id}__${user.uid}` : '';
  ...
  if (!job.userId) return null; // Community listings keep their direct-contact flow.
  if (!user) {
    return <div className="mt-2 text-[11px] text-slate-500">سجّل الدخول من أعلى الصفحة للتقديم عبر NEXT JOB.</div>;
  }
  if (job.userId === user.uid) return null;
  if (!candidate) {
    return <div className="mt-2 text-[11px] text-amber-700">أنشئ ملفك المهني أولًا لربط طلب التقديم بملفك.</div>;
  }
  ...
  const submit = async () => {
    ...
    try {
      await runTransaction(db, async tx => {
        const [existing, jobSnap] = await Promise.all([tx.get(ref), tx.get(doc(db, 'jobs', job.id))]);
      ...
      <div className="mt-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px]">
        <div className="flex items-center justify-between gap-2">
          ...
          {application.status !== 'withdrawn' && application.status !== 'rejected' && (
            <button disabled={busy} onClick={withdraw} className="text-slate-500 hover:text-rose-700 flex items-center gap-1"><XCircle className="w-3.5 h-3.5" />سحب</button>
          )}
        </div>
  ...
  return (
    <div className="mt-2">
      <button disabled={busy || job.status === 'closed'} onClick={submit} className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-teal-700 px-3 py-2 text-xs font-bold text-white hover:bg-teal-800 disabled:opacity-50">
        {busy ? <CheckCircle2 className="w-3.5 h-3.5 animate-pulse" /> : <Send className="w-3.5 h-3.5" />}
