...
import { Briefcase, UserPlus, FileText, Bookmark, BookOpen, LogIn, LogOut, PlusCircle, Users } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-slate-100/80 p-1.5 rounded-2xl border border-slate-200/60">
            ...
            >
              ...
              <span>المحفوظات</span>
              {savedCount > 0 && (
                <span className="w-5 h-5 bg-emerald-600 text-white rounded-full text-xs flex items-center justify-center font-bold">
                  {savedCount}
                </span>
              )}
            </button>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* Quick Action: Post Candidate CV */}
            <button
              id="btn-nav-post-candidate"
              onClick={onOpenPostCandidate}
              className="hidden lg:flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors"
            >
              <UserPlus className="w-4 h-4 text-emerald-600" />
              <span>أنشئ ملفك المهني</span>
            </button>

            {/* Main Action: Post Job */}
            <button
              id="btn-nav-post-job"
              onClick={onOpenPostJob}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-sm shadow-emerald-600/30 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              <PlusCircle className="w-4 h-4" />
              <span>أعلن عن وظيفة مجاناً</span>
            </button>
            ...
            {user ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button
                  id="btn-nav-profile"
                  onClick={onLogin}
                  title="عرض الملف الشخصي وإدارة الحساب"
                  className="flex items-center gap-2 p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl bg-slate-100/80 hover:bg-slate-200/80 border border-slate-200 transition-colors"
                >
              ...
              <button
                id="btn-nav-login"
                onClick={onLogin}
                className="flex items-center gap-1.5 px-3.5 py-2 text-xs sm:text-sm font-bold text-slate-700 bg-slate-100/90 hover:bg-emerald-50 hover:text-emerald-800 rounded-xl border border-slate-200 transition-colors"
              >
        ...
        <div className="flex md:hidden items-center justify-around py-2.5 border-t border-slate-100">
          <button
            ...
            className={`flex flex-col items-center gap-1 text-xs font-semibold ${
              activeTab === 'jobs' ? 'text-emerald-600 font-bold' : 'text-slate-500'
            }`}
          >
          ...
          <button
            ...
            onClick={() => setActiveTab('candidates')}
            className={`flex flex-col items-center gap-1 text-xs font-semibold ${
              activeTab === 'candidates' ? 'text-emerald-600 font-bold' : 'text-slate-500'
            }`}
          >
          ...
          <button
            ...
            onClick={() => setActiveTab('guide')}
            className={`flex flex-col items-center gap-1 text-xs font-semibold ${
              activeTab === 'guide' ? 'text-emerald-600 font-bold' : 'text-slate-500'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 text-xs font-semibold relative ${
              activeTab === 'saved' ? 'text-emerald-600 font-bold' : 'text-slate-500'
            }`}
          >
          ...
          <button
            id="mobile-nav-post-cv"
            onClick={onOpenPostCandidate}
            className="flex flex-col items-center gap-1 text-xs font-semibold text-emerald-700"
          >
