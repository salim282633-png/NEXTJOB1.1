...
import { Briefcase, HeartHandshake, Search, ShieldAlert, Sliders, X } from 'lucide-react';
...
interface Props {
  ...
  onApprovePendingJob?: (job: Job) => void;
  onRejectPendingJob?: (job: Job) => void;
...
export const AdminAndSEOEngineModal: React.FC<Props> = ({
  isOpen,
  onClose,
  pendingJobs = [],
  ...
  onApprovePendingJob = () => {},
  onRejectPendingJob = () => {},
  onApproveCommunityJob = () => {},
  onRejectCommunityJob = () => {},
  onResolveReport = () => {},
  jobsCount,
  candidatesCount,
  jobs = [],
  ...
  const [tab, setTab] = useState<'seo'|'gsc'|'jobReview'|'community'|'reports'|'monetization'>('jobReview');
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto" dir="rtl">
      <div className="max-w-6xl mx-auto my-4 bg-slate-50 rounded-3xl overflow-hidden shadow-2xl border">
        <div className="p-5 bg-slate-900 text-white flex justify-between items-center">
          <div>
            <h2 className="font-black text-lg flex gap-2"><Sliders className="w-5 h-5 text-indigo-400" />لوحة الإدارة وSEO الإنتاجية</h2>
            <p className="text-xs text-slate-400 mt-1">الوظائف المباشرة الجديدة لا تظهر للعامة قبل اعتماد الإدارة.</p>
          </div>
          <button onClick={onClose} aria-label="إغلاق لوحة الإدارة"><X className="w-5 h-5" /></button>
        </div>

        <div className="flex gap-1 overflow-x-auto bg-white border-b p-2 text-xs font-bold">
          <button onClick={()=>setTab('jobReview')} className={`px-3 py-2 rounded-xl ${tab==='jobReview'?'bg-emerald-700 text-white':''}`}><Briefcase className="w-3.5 h-3.5 inline ml-1" />مراجعة الوظائف ({pendingJobs.length})</button>
        ...
        <div className="p-5 sm:p-7 min-h-[420px]">
          {tab==='jobReview' && (
            <Queue items={pendingJobs} empty="لا توجد وظائف مباشرة بانتظار المراجعة.">
              {job => (
                <div className="bg-white border rounded-2xl p-4 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div>
                      <strong className="text-sm">{job.title}</strong>
                      <p className="text-xs text-slate-500 mt-1">{job.company} · {job.city} · {job.jobType}</p>
                      <p className="text-xs mt-2 leading-6 whitespace-pre-line">{job.description}</p>
                      {job.requirements?.length ? <p className="text-[11px] mt-2 text-slate-600">الشروط: {job.requirements.join(' · ')}</p> : null}
                      <p className="text-[11px] mt-2 text-slate-500">التواصل: <span dir="ltr">{job.phone}</span> · الراتب: {job.salary || 'يحدد لاحقًا'}</p>
                      <p className="text-[11px] mt-2 text-emerald-700 font-bold">إقرار الامتثال: {job.complianceAccepted ? 'مسجل' : 'غير موجود'}</p>
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button onClick={()=>onApprovePendingJob(job)} className="px-3 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold">اعتماد ونشر</button>
                      <button onClick={()=>onRejectPendingJob(job)} className="px-3 py-2 bg-rose-50 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold">رفض</button>
                    </div>
                  </div>
                </div>
              )}
